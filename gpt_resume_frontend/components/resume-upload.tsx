...
const ResumeUpload = () => {
  ...
  const handleDialogSubmit = async (jobTitle: string, jobDescription: string) => {
    if (!files.length) {
    ...
    try {
      ...
      const response = await axios.post(
        "http://127.0.0.1:8000/api/post-resume-with-job/",
        formData,
        {
          headers: {
      ...
      router.push(`/result/${response.data.job_u_id}`);
    } catch (error) {
      ...
      <div {...getRootProps({ className: "dropzone" })}>
        ...
        <div className="mx-auto border-[#5F5ADB] m-5 w-[40%] p-5 rounded-lg border-[2px] flex flex-col items-center justify-around gap-3 cursor-pointer">
          ...
          >
            <g opacity="0.7">
              <path
                d="M26.6666 26.6667L19.9999 20L13.3333 26.6667"
                stroke="#121212"
                strokeWidth="3.33333"
                strokeLinecap="round"
